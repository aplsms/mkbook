...
#  pip3 install pymad mutagen sox typing_extensions numpy
...
for filename in FILENAMES:
    ...
    length1 = sox.file_info.duration(filename) # don't use mutagen here, because it isn't very precise
    length  = mad.MadFile(filename).total_time() / 1000.

    logging.debug(f"File {filename} sox length is {length1} sec")
    logging.debug(f"File {filename} mad length is {length} sec")
